 ...
 * Description: ShmemCommMoeDistributeDispatchV2 tiling function implementation file
 * History: 2025-12-16 create ShmemCommMoeDistributeDispatchV2 tiling function implementation file
 */
...
#include "../op_kernel/shmem_moe_distribute_dispatch_v2_tiling.h"
...
namespace optiling {
static void PrintTilingDataInfo(const char *nodeName, ShmemMoeDistributeDispatchV2TilingData &tilingData)
{
...
static ge::graphStatus CheckAndSetGroupInfo(const gert::TilingContext *context, const char *nodeName,
                                            ShmemMoeDistributeDispatchV2TilingData &tilingData)
{
                                                    ...
                                                    bool &isSetCommAlg)
{
    ...
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchV2 isSetCommAlg = %d\n", isSetCommAlg);
    tilingData.moeDistributeDispatchV2Info.zeroComputeExpertNum = static_cast<int32_t>(zeroComputeExpertNum);
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchV2 zeroComputeExpertNum = %d\n",
...
static ge::graphStatus GetAttrAndSetTilingData(const gert::TilingContext *context, const char *nodeName,
                                               ShmemMoeDistributeDispatchV2TilingData &tilingData, bool &isSetCommAlg)
{
    auto attrs = context->GetAttrs();
    ...
    OP_LOGD(nodeName, "lastDim = %d, ShmemMoeDistributeDispatchV2 cumSumUBMinValue = %d\n", lastDim,
            tilingData.moeDistributeDispatchV2Info.cumSumUBMinValue);

    tilingData.moeDistributeDispatchV2Info.shmemptr = static_cast<uint64_t>(*shmemPtr);

    return ge::GRAPH_SUCCESS;
}

static bool CheckSharedAttrs(const char *nodeName, const ShmemMoeDistributeDispatchV2TilingData &tilingData)
{
...
static bool CheckCommAlgAttrs(const char *nodeName, const ShmemMoeDistributeDispatchV2TilingData &tilingData,
...
static ge::graphStatus CheckAttrs(const gert::TilingContext *context, const char *nodeName,
                                  ShmemMoeDistributeDispatchV2TilingData &tilingData, uint32_t &localMoeExpertNum,
                                  bool isActiveMask, bool isSetCommAlg)
{
    ...
    uint32_t sharedExpertRankNum = tilingData.moeDistributeDispatchV2Info.sharedExpertRankNum;
    uint64_t shmemptr = tilingData.moeDistributeDispatchV2Info.shmemptr;
    ...
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchV2 *globalBsPtr = %ld, bs = %ld, epWorldSize = %u\n", *globalBsPtr,
            xDim0, epWorldSize);
    OP_TILING_CHECK(
...
static ge::graphStatus CheckTensorShape(const gert::TilingContext *context, const char *nodeName,
                                        ShmemMoeDistributeDispatchV2TilingData &tilingData, const uint32_t quantMode,
...
static void SetHcommCfg(const gert::TilingContext *context, ShmemMoeDistributeDispatchV2TilingData *tiling)
{
    auto attrs = context->GetAttrs();
    // auto groupEpPtr = attrs->GetAttrPointer<char>(static_cast<int>(ATTR_GROUP_EP_INDEX));
    // auto groupTpPtr = attrs->GetAttrPointer<char>(static_cast<int>(ATTR_GROUP_TP_INDEX));
    ...
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchV2 groupEp = %s, groupTp = %s", groupEp.c_str(), groupTp.c_str());
...
static ge::graphStatus CheckWinSize(ShmemMoeDistributeDispatchV2TilingData &tilingData, const char *nodeName,
...
static ge::graphStatus SetAivInfo(gert::TilingContext *context, ShmemMoeDistributeDispatchV2TilingData *tilingData,
                                  const char *nodeName)
{
...
static ge::graphStatus GetParamsAndSetTilingData(const gert::TilingContext *context,
                                                 ShmemMoeDistributeDispatchV2TilingData *tilingData, bool &isActiveMask,
...
static ge::graphStatus MoeDistributeDispatchA3TilingFuncImpl(gert::TilingContext *context)
{
    const char *nodeName = context->GetNodeName();
    ShmemMoeDistributeDispatchV2TilingData *tilingData =
        context->GetTilingData<ShmemMoeDistributeDispatchV2TilingData>();
    ...
    OP_LOGI(nodeName, "Enter ShmemMoeDistributeDispatchV2 tiling check func.");
...
static ge::graphStatus ShmemMoeDistributeDispatchV2TilingFunc(gert::TilingContext *context)
{
    ge::graphStatus ret;
...
static ge::graphStatus TilingParseForShmemMoeDistributeDispatchV2(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ShmemMoeDistributeDispatchV2)
    .Tiling(ShmemMoeDistributeDispatchV2TilingFunc)
    .TilingParse<MoeDistributeDispatchCompileInfo>(TilingParseForShmemMoeDistributeDispatchV2);
}  // namespace optiling
