---
id: kernel-fused-ln-gelu
title: Fused LayerNorm + GELU (Hopper / H200)
type: kernel
architectures:
- sm90
tags:
...
performance_claims: []
blackwell_relevance: Same fused pattern on SM100; transformer block fusion.
operator_purpose: speedup
what_it_does: 'Fused LayerNorm+GELU: normalize then activate in one kernel (vs torch
  2-op).'
h200_validation:
  date: 2026-07-21 (phase2; self-reported summary)
  correctness: PASS — err ~1e-6 vs torch(layer_norm+gelu).
  result: 1.81x-2.29x faster than torch (2 ops fused into 1 pass).
...
## Overview
Fuses LayerNorm (mean/var reduction, normalize, scale+shift) and GELU-tanh
activation into one kernel — one read + one write of x, vs torch's separate
`layer_norm` then `gelu` (2 passes + intermediate).

```python
@triton.jit
def fused_ln_gelu(x_ptr, w_ptr, b_ptr, o_ptr, eps, N, BLOCK_N):
    ...
    xn=(x-mean)*rrms*w+b          # LayerNorm output
    c=0.7978845608028654; inner=c*(xn+0.044715*xn*xn*xn)
    e2=tl.exp(2.0*inner); tanh=(e2-1.0)/(e2+1.0)
    tl.store(o_ptr+row*N+offs, (0.5*xn*(1.0+tanh)).to(o_ptr.dtype.element_ty), mask=mask)  # GELU
```
## H200 measured: 1.81x-2.29x faster than torch.
